        ...
        <div className="grid grid-cols-1 gap-16 md:grid-cols-3 md:gap-8 lg:gap-12">
          {points.map((point, i) => (
            <Reveal
              key={point.index}
              delayMs={i * 100}
              className={`flex flex-col gap-6 ${point.offset}`}
            >
              <ImagePlaceholder
                src={point.src}
                alt={point.alt}
                sizes="(min-width: 1024px) 28vw, (min-width: 768px) 30vw, 100vw"
                imageClassName={`object-top ${point.imageClassName || ""}`}
                label={point.image}
              ...
              <div className="flex flex-col gap-4 border-t-2 border-foreground pt-5">
                <div className="flex items-center justify-between">
                  ...
                  <p className="font-display text-h2 font-black text-foreground/15">
                    {point.index}
                  </p>
                </div>
